public class Manager {
  ...
  void init() {
    ...
    if (arduino != null) {
      for (int i = 0; i < this.voiceCount (); i++) {
        arduino.pinMode( this.getVoice(i).readPort, Arduino.INPUT);
        arduino.pinMode( this.getVoice(i).lightPort, Arduino.OUTPUT);
  ...
  void update() {
    ...
    for (int i = 0; i < this.voiceCount (); i++) {
      ...
      this.getVoice(i).update();
      
      if (arduino != null) {
        
        this.getVoice(i).updateFilter( arduino.digitalRead(this.getVoice(i).readPort) );
